...
try {
    // Buscar a Ofensiva (Foguinho) para o Cabeçalho
    ...
    // Buscar Frentes e Tópicos com contagem de questões ativas no banco
    ...
    die("Erro ao carregar configurações do simulado: " . $e->getMessage());
}
?>
<!DOCTYPE html>
<html lang="pt-BR">
<head>
  ...
  <style>
    ...
    /* CABEÇALHO (Herdado do Dashboard para consistência máxima) */
    ...
    .nav-dash { padding: 12px 0; max-width: 1200px; margin: 0 auto; display: flex; justify-content: space-between; align-items: center; }
    ...
    /* SELETORES DE QUANTIDADE (Cards estilo Radio Button) */
    ...
    .label-card { 
      flex: 1; min-width: 100px; text-align: center; padding: 20px 10px; 
      background: var(--bg-global); border: 2px solid var(--borda); border-radius: 16px; 
      cursor: pointer; transition: all 0.2s ease; font-weight: 800; font-size: 1.3rem; color: var(--texto-secundario);
    }
    ...
    /* SELETORES DE MATÉRIA (Checkboxes elegantes) */
    ...
    .topico-item.desativado:hover { border-color: var(--borda); }
    ...
    .btn-gerar { 
      background: linear-gradient(135deg, var(--roxo-base), #4f46e5); color: white; border: none; 
      padding: 18px 50px; font-size: 1.2rem; font-weight: 800; border-radius: 50px; cursor: pointer; 
      box-shadow: 0 10px 25px rgba(79, 70, 229, 0.4); transition: all 0.3s ease; letter-spacing: 0.02em;
    }
...
<body class="dash-body">
  ...
  <header class="topo-dash">
    <div class="nav-dash container">
      ...
      <div style="display: flex; align-items: center; gap: 20px;">
        ...
        <a href="dashboard.php" style="color: var(--roxo-base); text-decoration: none; font-weight: 700; font-size: 0.95rem; transition: opacity 0.2s;">Sair do Modo Prova</a>
      </div>
    </div>
  </header>
  ...
  <form action="prova.php" method="POST" id="formSimulado">
    <div class="container-simulado">
      ...
      <div class="hero-simulado">
        ...
        <p class="subtitulo-hero">Escolha o tamanho do desafio e filtre apenas os tópicos que deseja revisar. A prova será gerada aleatoriamente em formato ENEM.</p>
      </div>

      <div class="secao-config">
        <h2 class="titulo-secao">1. Qual o tamanho do desafio?</h2>
        <div class="grid-quantidades">
          <?php $opcoes = [10, 15, 20, 25, 30, 50]; ?>
          <?php foreach ($opcoes as $idx => $qtd): ?>
      ...
      <div class="secao-config">
        ...
        <p style="color: var(--texto-secundario); margin-top: -15px; margin-bottom: 30px; font-size: 0.95rem;">Deixe tudo marcado para um Simulado Geral ou selecione tópicos específicos.</p>
            ...
            <div class="grid-topicos" id="grid_frente_<?php echo $frente_id; ?>">
              <?php foreach ($frente['topicos'] as $topico): ?>
                ...
                <label class="topico-item <?php echo $desativado ? 'desativado' : ''; ?>">
                  <input type="checkbox" name="topicos[]" value="<?php echo $topico['id']; ?>" class="check-custom checkbox-frente-<?php echo $frente_id; ?>" 
                         <?php echo $desativado ? 'disabled' : 'checked'; ?>>
                  <div class="topico-info">
                    <span class="topico-nome"><?php echo htmlspecialchars($topico['nome']); ?></span>
                    <span class="topico-badge">
                      <?php echo $desativado ? 'Sem questões cadastradas' : $topico['qtd'] . ' questões no banco'; ?>
                    </span>
                  </div>
                </label>
              <?php endforeach; ?>
  ...
  <script>
    // Função para Selecionar/Desmarcar todos os tópicos de uma frente rapidamente
    function toggleFrente(frenteId, btnElement) {
        ...
        if (marcados === checkboxes.length) {
            ...
            btnElement.style.borderColor = "var(--borda)";
            btnElement.style.color = "var(--texto-secundario)";
        } else {
            checkboxes.forEach(cb => cb.checked = true);
            btnElement.innerText = "Desmarcar Tudo";
            btnElement.style.borderColor = "var(--roxo-base)";
            btnElement.style.color = "var(--roxo-base)";
        }
    }

    // Prevenção de prova sem tópicos selecionados
    document.getElementById('formSimulado').addEventListener('submit', function(e) {
        const marcados = document.querySelectorAll('input[name="topicos[]"]:checked').length;
        if (marcados === 0) {
            e.preventDefault();
            alert('⚠️ Precisas de selecionar pelo menos um tópico para gerar a prova!');
        }
    });
  </script>
</body>
